...
import { 
    Wallet, Calculator, ArrowRightLeft, ArrowRight, TrendingDown, Plus, X, 
    CheckCircle2, Trash2 
} from 'lucide-react';
...
const Budget: React.FC<BudgetProps> = ({ itinerary }) => {
    const [currency, setCurrency] = useState<'NOK' | 'EUR'>('NOK'); // Default NOK
    const [rate, setRate] = useState(11.4); // Approx rate NOK to EUR
    const [calcAmount, setCalcAmount] = useState('');
    const [calcMode, setCalcMode] = useState<'NOK_TO_EUR' | 'EUR_TO_NOK'>('NOK_TO_EUR'); // Default NOK to EUR
